...
class EmailServiceReal {
  constructor() {
    ...
    this.transporter = nodemailer.createTransport({
      ...
      secure: false,
      auth: {
        user: 'ethereal.user@ethereal.email', // Will be replaced
        pass: 'ethereal.pass' // Will be replaced
      }
    });
  ...
  async initTestAccount() {
    try {
      ...
      // Re-create transporter with test account credentials
      this.transporter = nodemailer.createTransport({
        host: 'smtp.ethereal.email',
        port: 587,
        secure: false,
      ...
      console.log('💡 Check the preview URL to see sent emails\n');
  ...
  async sendVerificationEmail(user, token) {
    ...
    const mailOptions = {
      from: `"Clocked" <${this.testAccount?.user || 'noreply@clocked.in'}>`,
      to: user.email,
    ...
    try {
      ...
      console.log('💡 Check the preview URL to view the email\n');
      return true;
    } catch (error) {
...
module.exports = new EmailServiceReal();
